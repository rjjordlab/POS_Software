    ...
    internal sealed class Configuration : DbMigrationsConfiguration<POS_aplicacao.Models.PosAppDB>
    {
        ...
        protected override void Seed(POS_aplicacao.Models.PosAppDB context)
        {
            ...
            var produtos = new List<Produtos> {
               new Produtos  {ProdutoID=1,  Nome="74LS47",            Categoria="74XX LOGIC ICS", Descrisao="BCD To 7 Segment Decoder"                                    , Quantidade=50, Preco=1.20, FornecedorID=1},
               new Produtos  {ProdutoID=2,  Nome="DM7407N",           Categoria="74XX LOGIC ICS", Descrisao="7407 HEX BUFFERS WITH HIGH VOLTAGE OPEN-COLLECTOR OUTPUTS"   , Quantidade=30, Preco=2.10, FornecedorID=3},
               new Produtos  {ProdutoID=3,  Nome="74AC139PC",         Categoria="74XX LOGIC ICS", Descrisao="74AC139 DUAL 1-OF-4 DECODER/DEMULTIPLEXER"                   , Quantidade=50, Preco=1.49, FornecedorID=5},
               new Produtos  {ProdutoID=4,  Nome="SN74AC14N",         Categoria="74XX LOGIC ICS", Descrisao="74AC14 HEX SCHMITT-TRIGGER INVERTER"                         , Quantidade=40, Preco=1.25, FornecedorID=4},
               new Produtos  {ProdutoID=5,  Nome="74F04PC",           Categoria="74XX LOGIC ICS", Descrisao="HEX INVERTER, in 14-pin DIP package"                         , Quantidade=50, Preco=2.95, FornecedorID=6},
               new Produtos  {ProdutoID=6,  Nome="TC74HC595AP",       Categoria="74XX LOGIC ICS", Descrisao="74HC595 8-BIT SHIFT REGISTER LATCH, in 16-pin Dip package"   , Quantidade=40, Preco=5.05, FornecedorID=1},
               new Produtos  {ProdutoID=7,  Nome="SN74LS244N",        Categoria="74XX LOGIC ICS", Descrisao="74LS244 OCTAL BUFFER W/ TRI-STATE OUTPUTS"                   , Quantidade=20, Preco=3.20, FornecedorID=1},
               new Produtos  {ProdutoID=8,  Nome="SN74AC240N",        Categoria="74XX LOGIC ICS", Descrisao="74AC240 OCTAL BUFFER W/ TRI-STATE OUTPUTS"                   , Quantidade=80, Preco=4.50, FornecedorID=2},
               new Produtos  {ProdutoID=9,  Nome="TC74HC4053AP(F)",   Categoria="74XX LOGIC ICS", Descrisao="MUX/DEMUX TRIPLE 2X1"                                        , Quantidade=90, Preco=2.36, FornecedorID=6},
               new Produtos  {ProdutoID=10, Nome="MM74HCT240N",       Categoria="74XX LOGIC ICS", Descrisao="74HCT240 OCTAL INVERTER W/ TRI-STATE OUTPUTS"                , Quantidade=10, Preco=1.15, FornecedorID=5},
               new Produtos  {ProdutoID=11, Nome="MM74HC165N",        Categoria="74XX LOGIC ICS", Descrisao="74HC165 PARALLEL-IN/SERIAL-OUT 8-BIT SHIFT REGISTER"         , Quantidade=15, Preco=3.10, FornecedorID=4}
            };
            ...
            var vendas = new List<Vendas> {
               new Vendas  {VendaID=1, ClienteID=5, DataVenda="15-02-2017", TotalPago=14.40, EmpregadoID=1 }
            };

            vendas.ForEach(vv => context.Vendas.Add(vv));
            context.SaveChanges();
            // ############################################################################################
            // adiciona VendasDetalhes
            var vendasdetalhes = new List<VendasDetalhes> {
               new VendasDetalhes  {DetalheID=1, VendaID=1, ProdutoID=4, Quantidade=5, Preco=1.25, TotalAPagar=6.25 },
               new VendasDetalhes  {DetalheID=2, VendaID=1, ProdutoID=11, Quantidade=1, Preco=3.10, TotalAPagar=3.10 },
               new VendasDetalhes  {DetalheID=2, VendaID=1, ProdutoID=6, Quantidade=1, Preco=5.05, TotalAPagar=5.05 }
            };

            vendasdetalhes.ForEach(hh => context.VendasDetalhes.Add(hh));
            context.SaveChanges();
            // ############################################################################################
            // adiciona LoginAcesso
            var loginacesso = new List<LoginAcesso> {
               new LoginAcesso  {AcessoID=1234, EmpregadoID=1, Senha="pass123!" },
               new LoginAcesso  {AcessoID=7708, EmpregadoID=2, Senha="Rosa15" },
               new LoginAcesso  {AcessoID=1924, EmpregadoID=3, Senha="Melo12" },
               new LoginAcesso  {AcessoID=8074, EmpregadoID=4, Senha="Silva65" },
               new LoginAcesso  {AcessoID=9749, EmpregadoID=5, Senha="Oliveira14" }
            };

            loginacesso.ForEach(ll => context.LoginAcesso.Add(ll));
            context.SaveChanges();
        }
    }
}